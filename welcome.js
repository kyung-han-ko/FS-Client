	...
	if (passwordRegex.test(passwordValue)) {
		...
		resultDiv.innerHTML = '최소 8자 ~ 최대 16자 , 영어, 숫자 , 특수 문자를 포함해야 합니다.';
	...
	document.getElementById("emailplease").addEventListener("click", function(){
		fetch("http://localhost:8080/post", {
			...
			if(email.value === ""){
				...
				swal("이메일 발송이 완료되었습니다","인증번호 확인 후 입력해주세요","success")
			}
		...
		function checkSend(res){
			...
			if(res.success){
				swal("이메일 인증 완료","이메일 인증에 성공하였습니다 다음 단계로 이동해주세요","success")
				testBox.style.display = "block";
			} else {
				swal("인증실패","이메일 인증에 실패했습니다 인증번호를 다시 확인하세요","error")
