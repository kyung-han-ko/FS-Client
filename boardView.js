    ...
    if (userId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                .then(function (res) {
                    if (res.success) {
                        Swal.fire('Delete Success', '게시판이 성공적으로 삭제되었습니다.', 'success').then(()=> {
                            window.location.href = "board.html";
                        })
                    } else {
                        Swal.fire('Delete Failed', '게시판 삭제에 실패하였습니다.', 'error');
                    }
                })
                .catch(function (err) {
                    console.error('에러:', err);
...
updateButton.addEventListener("click", function () {
    ...
    .then(function (res) {
        if (res.success) {
            Swal.fire('Update Success', '게시물이 성공적으로 수정되었습니다.', 'success').then(()=> {
            ...
            res.result.forEach(reply => {
                ...
                deleteButton.addEventListener("click", function () {
                    swal.fire({
                        ...
                        if (result.isConfirmed) {
                            fetch(`http://localhost:8080/deleteUserReply?replyid=${reply.replyid}`, {
                                method: "post",
                                headers: {
                                    "Content-Type": "application/json",
                                },
                            ...
                            .then(function (res) {
                                if (res.success) {
                                    swal.fire("삭제 완료!", "댓글이 성공적으로 삭제되었습니다.", "success").then(()=> {
                                        location.reload();
                                    });
                                } else {
                                }
                ...
                editButton.addEventListener("click", function () {
                    fetch(`http://localhost:8080/getUserModifyReply?replyid=${reply.replyid}`, {
                        method: "get",
                        headers: {
                            "Content-Type": "application/json",
                        },
                ...
                // 댓글 수정 로직, 클릭하면 시작됨
                const updateButtonReply = document.getElementById("updateButtonReply")
                const replytext = document.getElementById("replytext")
                ...
                updateButtonReply.addEventListener("click", function () {
                    swal.fire({
                        ...
                        if (result.isConfirmed) {
                            fetch(`http://localhost:8080/modifyUserReply?replyid=${reply.replyid}`, {
                                method: "post",
                                headers: {
                                    "Content-Type": "application/json",
                                },
                            ...
                            .then(function (res) {
                                if (res.success) {
                                    swal.fire("수정 완료!", "댓글이 성공적으로 수정되었습니다.", "success").then(()=> {
                                        location.reload();
                                    });
                                } else {
                                }
